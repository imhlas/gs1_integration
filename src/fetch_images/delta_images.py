# src/fetch_images/delta_images.py
# Vastaa datan (urlien) hausta Deltasta


from typing import List, Dict, Iterator, Optional, Sequence
from pyspark.sql import functions as F
from src.config import *

def _base_df(spark, curated_items_path: str, gtins: Optional[Sequence[str]] = None):
    df = (
    ...
    # 🔹 UUSI: rajaa GTIN-listan mukaan, jos annettu
    if gtins:
        df = df.where(F.col("GTIN").isin(list(gtins)))
...
def get_image_rows_iter(
    ...
    - Jos gtins annetaan, suodattaa vain näihin GTIN-arvoihin.
    """
    df = _base_df(spark, curated_items_path, gtins=gtin_filter)
    if limit:
        df = df.limit(limit)
