...
- **ROLLBACK 2026-05-20 myöhemmin:** kytkentä peruttu samana päivänä koska `rembg`-paketin asennus tappaa toistuvasti Databricks-kernelin Runtime 16.4 Standard -clusterilla.
  - Yritykset: (1) `rembg>=2.0.50` cluster-libraryna → `Failure starting repl` -virhe. (2) cluster-libraryt poistettu, vain `%pip install -r requirements.txt` notebookissa → `numpy.dtype size changed` ABI-varoitus ja sitten `Kernel exited while we were waiting for the kernel_info_reply message`. (3) `numpy<2.0` + `rembg==2.0.50` pinnattu → `rembg 2.0.50` ei saatavilla Python 3.12 -ympäristöön (Runtime 16.4 vaatii vähintään `rembg>=2.0.55`). (4) `rembg>=2.0.55` ilman NumPy-pinnausta → kernel kaatui jälleen.
  - **Juurisyy ei vielä todistettu**, mutta epäilys: cluster on Standard Runtime (ei ML Runtime), eikä cluster-policy salli Runtime-vaihtoa eikä init-script-muutoksia (kuvakaappauksessa kaikki Edit-kentät lukittuina). `rembg`-asennus tai `onnxruntime`-riippuvuus näyttää tarvitsevan jotain system-tason kirjastoa jota ei voi asentaa kerneliä kaatamatta.
  - **Päätös**: jää image-processor erilliseksi repoksi (`iida.lassila/image-processor`) ja ajetaan se lokaalisti manuaalisesti toistaiseksi. Integraatio voidaan ottaa uudelleen ajan kanssa kun (a) ML Runtime saadaan käyttöön, (b) IT/admin asentaa init-scriptin libgl1:lle, tai (c) löydetään rembg-vaihtoehto joka ei tarvitse natiivi-riippuvuuksia.
  - **Hyödyllinen jäännös**: `.gitignore`-tiedosto jäi reposijaan (yleishyödyllinen). Tämä päiväkirjamerkintä jää historiana niin että seuraava yritys osaa välttää samoja umpikujia.
...
- **Lopullinen tulos (toteutuksen jälkeen):** 54 335 / 59 100 BASE_UNIT-tuotetta (**91.9 %**) sai myyntierämitat. Jakauma: 37 125 sai mitat CASE-emolta joinilla, 17 210 sai omat mittansa Case_*-sarakkeisiin (`IsDespatchUnit=true`, ei erillistä CASE-pakkausta), 4 765 jää NULL (aitoja kuluttajayksiköitä ilman myyntiräpakkausta Synkassa).
- **Toteutuksessa korjattu bugi:** alkuperäisellä toteutuksella enrich-funktio kaatui toisella ajolla `AMBIGUOUS_REFERENCE`-virheeseen, koska se kirjoitti samaan polkuun josta luki ja Case_*-sarakkeet kerääntyivät kaksoiskappaleena. Korjaus: drop stale Case_*-sarakkeet inputista ennen joinia.
