...
- [src/run_full_pipeline_with_images.py](src/run_full_pipeline_with_images.py) — ylätason "täysajo + kuvat + kuvankäsittely": ajaa `run_full_pipeline`, tyhjentää SharePoint-kirjaston, lataa kaikki kuvat, ja lopuksi kutsuu `sharepoint_image_processor.Orchestrator.run(mode="incremental")` joka poistaa taustat ja croppaa kuvat hyllykuva-formaattiin. Mittaa kestot kaikille kolmelle vaiheelle ja lähettää webhookin.

**Rikastus ja sivulataukset**
- [src/add_kesko_hierarchy_levels/enrich_kesko_categories.py](src/add_kesko_hierarchy_levels/enrich_kesko_categories.py) — joinaa Curated-tuotteet `dbo.KESKO_00_PRODUCT_HIERARCHY_LEVELS`-tauluun **kolmessa vaiheessa**:
  - **L0 (realtime, omat tuotteet)**: `dbo.KESKO_02_weekly_sales` (Lejosin myynnit Keskossa, viimeisen 4kk ajalta) joinataan KESKO_00:n L4-sarakkeeseen `TuoteryhmäID` -prefix-LIKE:llä. Curated-tuotteet matchataan EAN:llä (`GTIN_NO_LEADING_ZEROS == KESKO_02.EAN`). Omille tuotteille saadaan reaaliaikainen kategoria ilman KESKO_00:n 4kk-viivettä. Pieni tulosjoukko (~300–400 riviä), kysely ~70–80 s, käytetään `F.broadcast`-hintaa.
  - **L1 (viivästetty)**: suora `Curated.GTIN == KESKO_00.GTIN`.
  - **L2 (fallback)**: `Curated.GTIN_NO_LEADING_ZEROS == KESKO_00.GTIN`.
  - Coalesce-järjestys **L0 → L1 → L2** → `PRODUCT_HIERARCHY_LEVEL_2/3/4`. Lisäksi uusi sarake `KESKO_CATEGORY_SOURCE` (`realtime` / `kesko00_direct` / `kesko00_no_leading_zeros` / `NULL`) kertoo mistä mapping saatiin.
  - Suodattaa kaikissa kolmessa vaiheessa pois `EXCLUDE_L2` ja `EXCLUDE_L3_BY_L2` -kategoriat. Kirjoittaa Gold-tason `CURATED_ITEMS_WITH_KESKO`-poluun.
...
- [sharepoint_image_processor/](sharepoint_image_processor/) — **kuvankäsittelyvaihe** (integroitu erillisestä `image-processor`-repostasta, 20 .py-tiedostoa). `Orchestrator.run()` (async) lukee saman SharePoint-kirjaston "GS1 Tuotekuvat", poistaa kuvien taustat ML-mallilla (`rembg` + `u2net`), croppaa tight bounding boxiin (`Pillow`), kirjoittaa uutena PNG:nä takaisin samaan polkuun ja poistaa vanhan JPG/TIFF:n. Säilyttää SharePoint-metatiedot ja merkitsee `Processed=True` (idempotenssi). State DBFS:llä polussa `dbfs:/mnt/image-processor/state/{state,delta_token}.json` — `state.json` tallentaa tilastot, `delta_token.json` tallentaa SharePoint Change Tracking -tokenin inkrementaaliseen ajoon. Sisäänmenopiste `Orchestrator(AppConfig()).run(mode="incremental"|"full"|"sample", ...)`. Salaisuudet samat kuin gs1_integration: `gs1-kv/sharepoint-client-id` ja `sharepoint-client-secret` (sama App Registration).
...
- **Gold** `gs1/gold/items_with_kesko_levels` — Curated + `PRODUCT_HIERARCHY_LEVEL_2/3/4` + `KESKO_CATEGORY_SOURCE` (string: `realtime` / `kesko00_direct` / `kesko00_no_leading_zeros` / NULL) + `GTIN_NO_LEADING_ZEROS` + BASE_UNIT-riveille myyntierämitat: `Case_GTIN`, `Case_Depth_mm`, `Case_Width_mm`, `Case_Height_mm`, `Case_GrossWeight_g`, `UnitsPerCase` (Long). Muille tasoille (CASE, PALLET, kuluttajamonipakkaukset) `Case_*`-sarakkeet ovat NULL.
...
- **Riippuvuudet**: ks. [requirements.txt](requirements.txt). PySpark + Delta Lake (Databricks Runtime), `requests`, `pandas`, `mimetypes` (Databricks Runtime). Lisäksi `msal` (Microsoft Graph -token, gs1_integration ja sharepoint_image_processor jakavat tämän) sekä kuvankäsittelyn `aiohttp`, `Pillow`, `rembg`, `pydantic`, `pydantic-settings`, `typer`, `structlog`. Asennetaan joko cluster-tason kirjastoina (Compute → [cluster] → Libraries) tai notebookin ensimmäisessä solussa `%pip install -r requirements.txt` + `dbutils.library.restartPython()`.
