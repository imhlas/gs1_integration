...
- [src/add_kesko_hierarchy_levels/enrich_kesko_categories.py](src/add_kesko_hierarchy_levels/enrich_kesko_categories.py) — joinaa Curated-tuotteet `dbo.KESKO_00_PRODUCT_HIERARCHY_LEVELS`-tauluun **kolmessa vaiheessa**:
  ...
  - **Placeholder (omat tuotteet)**: jos omalta tuotteelta (`InfoProviderName == OWN_INFO_PROVIDER_NAME`, oletus `"Lejos Oy"`) puuttuu kategoria kaikilta kolmelta tasolta, **kaikkiin kolmeen** hierarkiasarakkeeseen kirjoitetaan `OWN_PLACEHOLDER_CATEGORY` (oletus `"99 - Luokittelematon (oma tuote)"`). Syy: kuvapipeline vaatii ei-tyhjän `PRODUCT_HIERARCHY_LEVEL_2`, joten ilman tätä omien uutuustuotteiden kuvat jäisivät lataamatta. Ehto on rivikohtainen → ei synny puoliksi täytettyjä rivejä. Placeholder korvautuu oikealla kategorialla automaattisesti seuraavassa ajossa, koska se on coalesce-järjestyksessä viimeisenä.
  - **Placeholderin esto**: apusarake `KESKO_KNOWN` (johdetaan **suodattamattomasta** KESKO_00:sta ja KESKO_02:sta, pudotetaan ennen kirjoitusta) estää placeholderin tuotteilta jotka Kesko tuntee mutta joiden kategoria on `EXCLUDE_L2`/`EXCLUDE_L3_BY_L2` -listalla. Ilman tätä tietoinen kategoriarajaus kiertyisi ohi omien tuotteiden kohdalla. Määrä raportoidaan lukuna `rows_own_suppressed`.
  - Coalesce-järjestys **L0 → L1 → L2 → placeholder** → `PRODUCT_HIERARCHY_LEVEL_2/3/4`. Lisäksi uusi sarake `KESKO_CATEGORY_SOURCE` (`realtime` / `kesko00_direct` / `kesko00_no_leading_zeros` / `own_placeholder` / `NULL`) kertoo mistä mapping saatiin. Huom: `rows_categorized`-mittari laskee vain oikeat kategoriat, placeholder raportoidaan erikseen `rows_via_own_placeholder`-lukuna.
